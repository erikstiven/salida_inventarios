...
<head>
    ...
    <script>
        function cargar_datos(id, lote, fcad, fela, cant) {
            if (cant > 0) {
                if (id == null) {
                    var id_var = 'loteProd';
                    var serie = 'serieProd';
                ...
                window.opener.document.getElementById(fela_var).value = fela;

                if (fcad == '' && fela == '') {
                    window.opener.document.getElementById(serie).value = lote;
                    window.opener.document.getElementById(id_var).value = null;
...
<body>
    ...
    $sql = "select t.tran_cod_tran from comercial.tran_sald_ini t where
    ...
    $sql_sp = "select * from sp_lotes_productos_web( $idEmpresa, $idSucursal, $dpef_cod_bode, '$fecha_ini', '$fecha_fin', '$dpef_cod_prod', '$dpef_cod_prod', '2' , $id_user, '$loteProd') ";
    $oIfx->Query($sql_sp);
    // echo $sql_sp;
    ...
    $Html_reporte .= '<tr>
							...
							<td class="bg-primary" align="center">Lote /Serie</td>
							<td class="bg-primary" align="center">MAC</td>
    ...
    $sql = "select t.prod_cod_prod, t.bode_cod_bode, t.cant_lote, t.num_lote,
			...
			from comercial.tmp_lote t where
			t.empr_cod_empr = $idEmpresa  and
    ...
    if ($oCon->Query($sql)) {
        if ($oCon->NumFilas() > 0) {
            do {
                ...
                $fec_ela = $oCon->f('fecha_ela_lote');
                $fec_cad = $oCon->f('fecha_cad_lote');
    ...
    if ($oIfx->Query($sql)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                if (!empty($fecha_cad)) {
                    $fecha_cad = ($fecha_cad);
                    $dia = restaFechas($fecha_server, ($oIfx->f('fcad')));
                }
                ...
                // mac del producto
                $sqlMacProd = "SELECT dmov_cod_serie FROM saedmov where dmov_cod_prod = '$prod_cod_prod' and dmov_cod_bode = $dpef_cod_bode and dmov_cod_lote = '$lote' and ( dmov_cod_serie != null OR  dmov_cod_serie != '') limit 1";
                $mac_producto = consulta_string_func($sqlMacProd, "dmov_cod_serie", $oIfxA, '');

                // stock pedido
                $sqlStockPedido = "select sum(dpef_cant_dfac) as dpef_cant_dfac 
                ...
                if ($i > 1) {
                    if ($array_prod[$i] != $array_prod[$i - 1]) {
                        ...
                        $Html_reporte .= '<tr height="20"  class="' . $sClass . '"
										onMouseOver="javascript:this.className=\'link\';"
										onMouseOut="javascript:this.className=\'' . $sClass . '\';"
										onclick="cargar_datos(' . $id . ', \'' . $lote . '\', \'' . $fecha_cad . '\',\'' . $fecha_ela . '\', \'' . $disponible . '\');" style="cursor: pointer;">';
                        ...
                        $Html_reporte .= '<td  align= "left" style="color: ' . $color . '; font-weight: bold;">' . $mac_producto . '</td>';
